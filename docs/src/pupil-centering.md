...
| BRM total NCP · WHMC | 84,335 | 4.54× | 6.57× |
| BRM total CP · WHMC | 44,130 | 133× | 194× |
| BRM total post-hoc position · WHMC | 118,655 | 232× | 91.5× |
| BRM total post-hoc gradient · WHMC | 118,674 | 215× | 84.5× |
| BRM total online position · WHMC | 34,526 | 235× | 318× |
| BRM total online gradient · WHMC | 52,951 | 159× | 224× |
...
The nine brms fits are reused from the verified comparison because their targets and sampler configurations are unchanged. All six BRM-total fits use the built-in implementation and repaired WHMC transport. The comparison measures gradient work, not identical wall cost per gradient. WHMC counts target-gradient requests. Native Stan uses a zero-contribution C++ counter compiled into the model; its sampler is unchanged. Retained increments match leapfrog counts plus one, and final process counts include initialization, warmup and the auto precursor.
...
All 15 rows have zero sampling divergences. The new total-NCP fit has minimum ESS 88.8 and maximum within-chain split R-hat 1.013; the online position and gradient fits have maxima 1.007 and 1.004. Across all quantities, mean differences from the total-CP fit are below three combined estimated MCSEs except one ordinary-NCP WHMC quantity at 3.08 and one native S2Z-auto quantity at 3.48. These are descriptive consistency checks, not convergence guarantees.

Excluding recovered population coefficients leaves 44 invariant quantities. Online position still gives 124× sampling efficiency and 168× total efficiency against that baseline set. Across ten recovery seeds, its minimum ESS ranges from 1,707 to 1,759; online gradient ranges from 1,874 to 1,903. The other new total rows have unchanged minima. The main conclusions do not depend on one recovery draw.
...
- [BRM declarations and scientific quantities](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/common.jl).
- [Independent target and recovery audit](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/audit.jl).
- [Six total fits and centering selection](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/total_whmc.jl).
- [Reused brms fits](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/reuse_brms.jl).
- [Complete comparison table](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/results/comparison.tsv).
- [Per-quantity ESS and MCSE](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/results/per_quantity_mcse.tsv).
- [Recovery sensitivity](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/results/recovery_seed_sensitivity.tsv).
- [brms model sources and original fit archives](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_total_effects/README.md).
- [New generated target and fit manifest](https://github.com/nsiccha/BayesianRegressionModels.jl/blob/ns/devibe/research/pupil_builtin_totals/README.md).
