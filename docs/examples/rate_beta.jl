...
# Run: julia --project=. rate_beta.jl   (from docs/examples/, after Pkg.instantiate())
...
b_g = @brm begin
    log(mu) ~ 1 + age2 + age3 + age4 + age5 + age6 + male + areaB + areaC + areaD + areaE + areaF
    effect(mu, Intercept) ~ Normal(0, 2.5)
    effect(mu, age2) ~ Normal(0, 2.5)
    effect(mu, age3) ~ Normal(0, 2.5)
    effect(mu, age4) ~ Normal(0, 2.5)
    effect(mu, age5) ~ Normal(0, 2.5)
    effect(mu, age6) ~ Normal(0, 2.5)
    effect(mu, male) ~ Normal(0, 2.5)
    effect(mu, areaB) ~ Normal(0, 2.5)
    effect(mu, areaC) ~ Normal(0, 2.5)
    effect(mu, areaD) ~ Normal(0, 2.5)
    effect(mu, areaE) ~ Normal(0, 2.5)
    effect(mu, areaF) ~ Normal(0, 2.5)
    phi ~ Exponential(1)
    claim ~ Gamma(phi, mu / phi)
end
sb = SBBRMI(b_g((; claim=ck, dummies...)); mod=@__MODULE__)
problem = StanBlocks.stan_instantiate(sb.model; path=joinpath(SCRATCH, ".out", "stan", "gamma_claim.stan"))
f = sample_fit(sb, problem, 3001)
open(joinpath(OUT, "gamma.json"), "w") do io
    JSON.print(io, f)
end
println("GAMMA_DONE")
flush(stdout)
...
# Lag needs the full AB>100 frame (1991 rows take 1990 predecessors), so
# re-read rather than shifting the year-filtered intercept frame above.
bdf = CSV.read(joinpath(SCRATCH, "data", "Batting.csv"), DataFrame)
bdf.bavg = Float64.(bdf.H) ./ Float64.(bdf.AB)
bdf = bdf[bdf.AB .> 100, :]
bdf.bavg_shift = [i == 1 ? missing :
    (bdf.playerID[i] == bdf.playerID[i-1] ? bdf.bavg[i-1] : missing)
    for i in 1:nrow(bdf)]
bdf0 = bdf[(bdf.yearID .> 1990) .& (bdf.yearID .< 2018) .&
    .!ismissing.(bdf.bavg_shift) .& isfinite.(bdf.bavg) .& isfinite.(bdf.bavg_shift), :]
