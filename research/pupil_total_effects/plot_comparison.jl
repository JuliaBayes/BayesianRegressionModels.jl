...
function plot_efficiency(output)
    ...
    spec=data(rows)*mapping(:value=>"Log scale",:model=>"";
        ...
        config(width=225,height=480,facet=(;linkxaxes=:none),
    ...
    println("EFFICIENCY_PLOT_COMPLETE\t",output)
end

function plot_comparison(output)
    plot_efficiency(output)
...
if abspath(PROGRAM_FILE)==@__FILE__
    if length(ARGS)==2 && ARGS[2]=="--efficiency-only"
        plot_efficiency(ARGS[1])
    else
