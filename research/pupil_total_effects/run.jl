...
function problem(controls, data; mean_prior=PTE.GaussianMean())
    J = length(data.ids)
    raw = PTE.PupilProblem(data,mean_prior)
...
function audit_reparametrization(data;mean_prior=PTE.GaussianMean())
    q = PTE.initial_position(data,mean_prior)
    controls = collect(range(0.0,1.0,length=40))
    p = problem(controls, data;mean_prior)
...
function diagnostic_rows(label, record, data)
    ...
    names = PTE.coordinate_names(data,get(record,:mean_prior,PTE.GaussianMean()))
    ...
    summary = (; arm=label, retained_draws=size(q,2), min_bulk_ess=minimum(bulk),
        ...
        min_total_coefficient_ess_per_1000_gradients=1000minimum(bulk[5:44])/record.sampling_gradients,
        min_common_bulk_ess_per_1000_gradients=1000minimum(bulk[1:44])/record.sampling_gradients,
...
function run_arm(label, controls, data, output; online=false, n_draws=2000, seed=1,
                 mean_prior=PTE.GaussianMean())
    target = problem(controls, data;mean_prior)
    initial = PTE.to_source(PTE.initial_position(data,mean_prior),controls,data)
    ...
    record = (; positions=convert(Matrix{Float64},fit.posterior_position),
        ...
        seed, mean_prior, requested_draws=n_draws,
        model_frame="log_tau_a,log_tau_b,gamma0,gamma1,total_A,total_B[,log_lambda]")
...
function main()
    ...
    mean_prior = prior_kind == "gaussian" ? PTE.GaussianMean() :
                 prior_kind == "student_mixture" ? PTE.StudentMixtureMean() :
                 error("Unknown PUPIL_INTERCEPT_PRIOR=$prior_kind")
    write_tsv(joinpath(output,"density_audit.tsv"),audit_model(PTE.PupilProblem(data,mean_prior)))
    transform_error = audit_reparametrization(data;mean_prior)
    ...
    changes = prior_kind == "gaussian" ?
        "independent random intercept/load effects; Normal(5651.9,2026.1) mean-intercept prior" :
        "independent random intercept/load effects; original Student-t mean-intercept prior via exact mixture"
    provenance = Dict("source_data_revision"=>PTE.DATA_REVISION,"data_sha256"=>PTE.DATA_SHA256,
        ...
        "changes_from_post"=>changes,
        "intercept_prior"=>prior_kind,
        ...
        "sampled_dimensions"=>LogDensityProblems.dimension(PTE.PupilProblem(data,mean_prior)),"groups"=>20,"observations"=>2228,
        "integrated_dimensions"=>2,"original_parameters_recovered"=>false,
        "posterior_scope"=>"same independent-RE pupil posterior for each intercept-prior comparison; forum model additionally has correlated REs",
        "comparison_baseline"=>"conventional brms NCP, run separately with brms_baseline.jl",
        "integrated_control"=>"fixed c=0 scaled total coefficients; not independent-standard-normal NCP",
        "offline_criterion"=>"log standard deviation plus inverse log-Jacobian; grid 0:0.01:1",
        "online_status"=>"disabled at user request 2026-09-15T14:23:49+02:00; historical failed fit preserved",
        "ess_scope"=>"all model-frame coordinates; common minimum excludes any mixture precision",
    ...
    baseline, ds = run_arm("scaled_total",zeros(40),data,output;mean_prior)
    summaries = [ds]
    ...
    partial, ds = run_arm("partial",selected,data,output;mean_prior)
    push!(summaries,ds); write_tsv(joinpath(output,"diagnostics.tsv"),summaries)
    write_tsv(joinpath(output,"workflow_costs.tsv"),[
        (; workflow="integrated_control",all_gradient_calls=baseline.all_gradient_calls),
