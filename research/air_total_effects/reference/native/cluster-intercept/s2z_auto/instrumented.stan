...
transformed data {
  ...
  vector<lower=0>[N_1] exposure_fisher_s2z_1;
  vector[2] H_s2z_1;
  for (i in 2:K) {
  ...
  exposure_fisher_s2z_1 = zeros_vector(N_1);
  for (n in 1:N) {
...
parameters {
  ...
  vector[N_1 - 1] z_s2z_1;  // partially centered orthonormal scalar S2Z coordinates
...
transformed parameters {
  ...
  real lprior = 0;
  real pupil_counter_tick = pupil_record_gradient(theta_s2z_active[1]);
  theta_s2z[1] = theta_s2z_active[1];
  theta_s2z[2] = fixed_s2z[1];
  log_det_partial_s2z_1 = 0.0;
  {
    vector[N_1] centered_partial_s2z = sum_to_zero_constrain_brms(z_s2z_1);
...
generated quantities {
  ...
  if (compute_rho_center_candidate_1) {
  {
    ...
    vector[N_1] relative_post_var_fisher_s2z;
    real restricted_prior_fraction_fisher_s2z = 1.0 - inv(N_1);
    real sum_relative_post_var_fisher_s2z = 0.0;
    for (j in 1:N_1) {
      real scaled_info_fisher_s2z = square(sd_1[1]) * obs_prec_fisher_s2z * exposure_fisher_s2z_1[j];
    ...
    for (j in 1:N_1) {
      ...
      rho_center_candidate_1[j, 1] = fmin(1.0, fmax(0.0, 1.0 - restricted_relative_post_var_fisher_s2z / restricted_prior_fraction_fisher_s2z));
      rho_center_candidate_1[j, 1] = rho_center_candidate_1[j, 1] / (rho_center_candidate_1[j, 1] + (1.0 - rho_center_candidate_1[j, 1]) * (sd_1[1]));
    }
    mean_rho_center_candidate_1[1] = mean(rho_center_candidate_1[, 1]);
  }
  } else {
