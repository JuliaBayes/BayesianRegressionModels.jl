...
# predict step (0 = first-order plug-in, as in ctsem; 3 / 5 = Gauss-Hermite moment matching).
...
#      (with `outdir`, each rung's constrained draws + PSIS weights are written there as CSV)

using BayesianRegressionModels, StanBlocks, LogDensityProblems, BridgeStan, Random, WarmupHMC
using Distributions: Normal, Exponential
import Statistics, PSIS

include(joinpath(@__DIR__, "ema_state_dependent.jl"))   # ema_sd triad, fixture, with_filter, model
...
function problem(panel, cfg)
    sb = SBBRMI(ema_state_dependent(with_filter(panel; cfg...)); mod=@__MODULE__)
