...
const output=path.join(root,process.argv[2]||'automatic-total-browser-v1'); await fs.mkdir(output,{recursive:true});
const server=http.createServer(async(req,res)=>{
 try {let name=decodeURIComponent(new URL(req.url,'http://localhost').pathname);if(!path.extname(name))name+='.html';
 const file=path.join(process.env.PREVIEW_DIST||path.join(root,'automatic-total-render-v1/build/1'),name);const body=await fs.readFile(file);
...
const results=[];
const pages=['pupil-centering','pupil-scale-centering','air-centering'];
try{
 ...
 for(const page of pages){
  ...
  if(JSON.stringify(before.efficiencyTables)!==JSON.stringify((page==='air-centering'?[14,14]:[15])))throw Error(JSON.stringify({page,before}));
  if(page==='pupil-centering'&&before.displayMath<4)throw Error('Pupil display equations missing');
  if(!before.sidebar.includes('Adaptive centering')||!before.sidebar.includes('Pupil: hierarchical residual SD'))throw Error('Category missing');
  if(before.panes.length!==(page==='air-centering'?8:4))throw Error('Backend panes missing');
 ...
 for(const page of pages){
  ...
  if(state.url!==`/${page}`||state.panes!==(page==='air-centering'?8:4))throw Error(JSON.stringify(state));
 }
 ...
 console.log('THREE_PAGE_BROWSER_COMPLETE');
}finally{
