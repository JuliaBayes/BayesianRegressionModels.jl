...
function online_loss_preview(input_dir; normalization=:minmax)
    ...
    plot = brm_centering_lossplot(rows; title, configurations=true, normalization,
        ylabel=normalization == :minmax ? "Loss (per-curve min–max [0, 1])" :
                                          "Position–gradient correlation (w₁ = 0)")
    ...
    envelope = Dict("schema" => "kb-aov/v1", "title" => title,
        "alt" => "Three independently sampled fits in columns, two GPs in rows, four basis functions per panel. Native WarmupHMC candidate scores, unit-weight replay on all 10,000 saved draws. Display normalization: $normalization per curve; raw correlations retained. $unavailable nonfinite or missing candidate values are gaps, retained in the source table.",
        "spec" => spec, "provenance" => Dict(
            "producer" => "BayesianRegressionModels:docs:adaptive-centering",
            "mode" => "preliminary", "base_commit" => base,
            "run" => "native-candidate-scoring-losses-saved-draws-$normalization-v2",
