...
const root='/home/n/scratch/kb-agent-tmp/BayesianRegressionModels-docs-adaptive-centering/claude-1000/-home-n--local-state-kb-agents-worktrees-BayesianRegressionModels-docs-adaptive-centering/54cbb8e1-8672-495f-a717-27389b0ab9be/scratchpad';
const output=path.join(root,process.argv[2]||'rbest-browser-v1'); await fs.mkdir(output,{recursive:true});
const server=http.createServer(async(req,res)=>{
 try {let name=decodeURIComponent(new URL(req.url,'http://localhost').pathname);if(!path.extname(name))name+='.html';
 const file=path.join(process.env.PREVIEW_DIST||path.join(root,'rbest-render-v1/build/1'),name);const body=await fs.readFile(file);
...
const results=[];
const pages=['rbest-centering','air-centering'];
try{
 ...
 for(const page of pages){
  ...
  if(JSON.stringify(before.efficiencyTables)!==JSON.stringify((page==='air-centering'?[14,14]:[18,18])))throw Error(JSON.stringify({page,before}));
  if(page==='rbest-centering'&&before.displayMath<1)throw Error('RBesT display equation missing');
  if(!before.sidebar.includes('Adaptive centering')||!before.sidebar.includes('RBesT MAP prior'))throw Error('Category missing');
  if(before.panes.length!==8)throw Error('Backend panes missing');
  await evaluate('document.querySelectorAll(".backend-comparison").forEach(root=>root.querySelectorAll(".backend-comparison__tab")[2].click())');
  if(page==='rbest-centering'){await evaluate("document.querySelector('mjx-container[display=\"true\"]').scrollIntoView({block:'center'})");await fs.writeFile(path.join(output,'rbest-equations.png'),Buffer.from((await c.send('Page.captureScreenshot')).data,'base64'));}
  await evaluate('document.querySelector(".backend-comparison").scrollIntoView({block:"start"})');
  await fs.writeFile(path.join(output,page+'-stan.png'),Buffer.from((await c.send('Page.captureScreenshot')).data,'base64'));
  const selected=await evaluate('[...document.querySelectorAll(".backend-comparison__panel:not([hidden])")].map(x=>x.dataset.backendPane)');
  if(selected.some(x=>x!=="stan"))throw Error(JSON.stringify({page,selected}));
 ...
 for(const page of pages){
  ...
  if(state.url!==`/${page}`||state.panes!==8)throw Error(JSON.stringify(state));
 }
 ...
 console.log('RBEST_PAGE_BROWSER_COMPLETE');
}finally{
