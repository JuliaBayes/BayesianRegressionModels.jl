...
## Sum-to-zero in RBesT: pull request 64

Sebastian Weber's Discourse post 32 in the [brms sum-to-zero / partial-centering thread](https://discourse.mc-stan.org/t/help-testing-brms-pr-for-sum-to-zero-and-partial-centering/41542/32) reports adding sum-to-zero to gMAP for about 5x efficiency, and about 10x with a still-local auto-centering. The public part is RBesT pull request 64 (`weberse2:issue-prep-1-12-0` -> `develop`, head `a5acbbc39c2cd620f0549159aa7c1991d4c28d6a`, `DESCRIPTION` 1.12-0), checked out at `<scratch>/novartis/RBesT-pr64` and built into its own R library. From its `inst/stan/gMAP.stan` and `R/gMAP.R` diff against 1.11-0:

- New `functions` block `zero_sum_basis(J)`: the Helmert basis `Q` with `Q'Q = I_{J-1}`, `1'Q = 0` (RBesT targets Stan 2.32, so no `sum_to_zero_vector`). Data gains `has_intercept` and `use_s2z` (option `RBesT.MC.s2z`, default `TRUE`). `s2z` is active only for Normal random effects, one `tau` stratum and an intercept.
- Parameters: `xi_eta` has `J - 1` entries (the contrast coordinates), plus a data-free `xi_abar[1] ~ N(0,1)`. The sampled intercept is `alpha = beta[1] + mean(eps)` with the widened prior `alpha ~ normal(m1, hypot(s1, tau/sqrt(J)))`; `theta = X*beta + Q * (ncp ? tau * xi_eta : g_s * xi_eta)`; the super-population `beta[1] = alpha - r (r (alpha - m1) + s1 xi_abar)` with `r = (tau/sqrt(J)) / hypot(s1, tau/sqrt(J))` is an exact conditional draw, written into `beta[1]` after `theta` is formed. The `ncp` switch still applies to the contrasts (`xi_eta ~ std_normal()` or `normal(0, tau/g_s)`).
- Default `adapt_delta` drops from 0.99 to 0.95 under S2Z (`.gmap_sampler_control`); legacy keeps 0.99. `xi_abar` joins the excluded parameters.
- His J = 4 simulation stress test (post 32): ESS per thousand gradients about 9.6 legacy, 43 S2Z, 42 S2Z-centered, 106 auto-centering; the last is not in the pull request.

Relation to BRM's exact totals: both integrate the data-free common shift of the group effects. RBesT samples `(alpha, J - 1 contrasts)` and recovers `beta[1]` conditionally; BRM samples the `J` totals under their exact joint prior `tau^2 I + s^2 11'` and recovers the population coefficient conditionally. The two are the same posterior under a linear change of coordinates (the audit checks this at mapped points); what the page measures on top is the centering control: RBesT's single global `ncp` switch on the contrasts against BRM's per-trial post-hoc and online selection.

## Protocol for this page

- One chain, seed 1, 10,000 retained draws per arm (the eight-schools/radon/HSGP convention; the model is small). Warmup per the sampler's own policy.
- Native arms per case from `capture.R` + `native.R`, all with the gradient counter of `pupil_scale_totals/support`: `rbest_ncp`, `rbest_cp` (RBesT 1.11 legacy program, RBesT's own control: adapt_delta 0.99, stepsize 0.01, max_treedepth 20, warmup 2000); `s2z_ncp`, `s2z_cp` (pull request 64 program, its control: adapt_delta 0.95, otherwise the same); `stan_ncp`, `stan_cp` (legacy program under CmdStan defaults: adapt_delta 0.8, max_treedepth 10, warmup 1000) for comparability with the brms baselines of the other pages. Thin 1 (RBesT's default thin 4 discards draws without saving gradients). All arms start from the same empirical per-trial point mapped into each program's coordinates.
- BRM arms from `run.jl` under WarmupHMC: `total_{ncp, cp, posthoc_position, posthoc_gradient, online_position, online_gradient}` on the exact-totals target, and `ordinary_{ncp, cp, posthoc_position, posthoc_gradient, online_position, online_gradient}` on the conventional block through the ordinary adaptive wrapper. Post-hoc arms charge the full NCP pilot.
- Scientific quantities: `population_mean` (`beta`, recovered conditionally where integrated), `between_trial_sd` (`tau`), and the `H` trial totals `theta_h`. Efficiency is min bulk ESS over these per thousand sampling gradients and per thousand total gradients, relative to `rbest_ncp` (what an RBesT 1.11 user gets today).
- BRM's conventional `centered_groups` samples the deviation `b_j ~ N(0, tau)` with the intercept kept separate (the wrapper's `c = 1`); RBesT's legacy centered form samples `theta_j` itself. The latter geometry is what BRM's exact totals at `c = 1` sample, with the intercept integrated. The page states this so "CP" is not read as one thing.
