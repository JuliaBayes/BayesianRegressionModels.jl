...
const GROUPED_PARTIAL_HSGP = @brm begin
    loc ~ 1 + hsgp(x; k=4, by=g, centeredness=c_hsgp)
    y ~ Normal(loc, 1)
end

const GROUPED_PARTIAL_HSGP_ANISO = @brm begin
    loc ~ 1 + hsgp(x, x2; k=(3, 4), c=(1.5, 2.0), iso=false, by=g,
                   centeredness=c_hsgp)
    y ~ Normal(loc, 1)
end

grouped_partial_df(; shift_x=0.0, c_hsgp=[0.0, 0.5, 1.0, 0.25]) =
    merge(gp_term_df(; shift_x), (; c_hsgp))

@testset "grouped HSGP accepts per-basis partial centering" begin
    df = grouped_partial_df()
    sb_partial = SBBRMI(GROUPED_PARTIAL_HSGP(df); mod=@__MODULE__)

    # The per-basis vector lands in the Stan data under a by-scoped key, and
    # no shared iid-normal weight block is allocated: the partial weights'
    # prior scale depends on the sampled hyperparameters, so the term
    # submodel declares them instead.
    @test sb_partial.data[:centeredness_hsgp_x_by_g] == [0.0, 0.5, 1.0, 0.25]
    @test !haskey(sb_partial.data, :zflat_hsgpw_x_g)
    @test sb_partial.data[:g_idx] == [1, 1, 1, 1, 2, 2, 2, 2]
    @test size(sb_partial.data[:PHI_hsgp_x_by_g]) == (GP_TERM_N, 4)

    code = StanBlocks.stan_code(sb_partial.model)
    @test occursin("hsgp_x_by_g_beta_flat ~ normal(0.0, " *
                   "rep_vector(exp(hsgp_x_by_g_centered_log_scale), n_g))", code)
    @test occursin("to_matrix(hsgp_x_by_g_beta_flat, hsgp_x_by_g_n_basis, n_g)", code)
    @test !occursin("zflat_hsgpw_x_g", code)
    @test StanBlocks.stan.transpiles(sb_partial.model)
    @test StanBlocks.stanc_check(code; warn_pedantic=false).ok

    # The pilot hookup: a select_hsgp_centeredness vector feeds the grouped
    # emission directly — the spectral scales are shared across groups.
    unit = reshape(collect(range(-1.2, 1.3; length=32)), 8, 4)
    logs = repeat(collect(range(-3.0, -1.0; length=8)), 1, 4)
    selection = select_hsgp_centeredness(unit, logs; candidates=0:0.5:1)
    @test length(selection.centeredness) == 4
    sb_pilot = SBBRMI(GROUPED_PARTIAL_HSGP(
        grouped_partial_df(; c_hsgp=selection.centeredness)); mod=@__MODULE__)
    @test sb_pilot.data[:centeredness_hsgp_x_by_g] == selection.centeredness
    @test StanBlocks.stan.transpiles(sb_pilot.model)
    @test StanBlocks.stanc_check(
        StanBlocks.stan_code(sb_pilot.model); warn_pedantic=false).ok

    # Replay keeps the program byte-identical and the centeredness vector.
    replay = reprocess(sb_partial, grouped_partial_df(; shift_x=0.25))
    @test StanBlocks.stan_code(replay.model) == code
    @test replay.data[:centeredness_hsgp_x_by_g] == [0.0, 0.5, 1.0, 0.25]

    # New-population prediction stays loudly unsupported, as for grouped HSGP.
    @test_throws "no cv-contagious size" reprocess(
        sb_partial, grouped_partial_df(); resample_groups=:g)

    # The default grouped emission is untouched: still the iid block.
    default_code = StanBlocks.stan_code(
        SBBRMI(grouped_hsgp_model(gp_term_df()); mod=@__MODULE__).model)
    @test occursin("zflat_hsgpw_x_g", default_code)
    @test !occursin("by_partial", default_code)

    # The anisotropic spelling selects the vector-rho sibling.
    sb_aniso = SBBRMI(GROUPED_PARTIAL_HSGP_ANISO(
        merge(gp_term_df(), (; c_hsgp=[0.25 for _ in 1:12]))); mod=@__MODULE__)
    aniso_code = StanBlocks.stan_code(sb_aniso.model)
    @test occursin("beta_flat ~ normal(0.0, " *
                   "rep_vector(exp(hsgp_x_x2_by_g_centered_log_scale), n_g))",
                   aniso_code)
    @test !occursin("zflat_hsgpw_x_x2_g", aniso_code)
    @test StanBlocks.stan.transpiles(sb_aniso.model)
    @test StanBlocks.stanc_check(aniso_code; warn_pedantic=false).ok

    @testset "BridgeStan finite density and gradient" begin
        if GP_RUN_BRIDGESTAN
            isdir(GP_TERM_CACHE) || mkpath(GP_TERM_CACHE)
            for sb in (sb_partial, sb_aniso)
                code = StanBlocks.stan_code(sb.model)
                problem = StanBlocks.stan_instantiate(
                    sb.model; path=joinpath(GP_TERM_CACHE, string(hash(code)) * ".stan"))
                dimension = LogDensityProblems.dimension(problem)
            ...
            @info "Skipping BridgeStan grouped-partial-HSGP runtime gate (BRM_GP_RUNTIME=0)"
        end
    end
end
